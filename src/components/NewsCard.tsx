...
export default function NewsCard({ post, variant = "large" }: NewsCardProps) {
  ...
  const getCategoriaClass = (catId: number | "default") => {
    ...
    return (
      categoriaMap[catId as keyof typeof categoriaMap] || categoriaMap.default
    );
  };
  ...
  return (
    <Link href={getPostUrl(post)} className="block">
      <article className="bg-fundo-card rounded-lg overflow-hidden shadow-card hover:shadow-card-hover transition-all transform hover:-translate-y-1 duration-300 cursor-pointer group border-[1px] border-gray-100">
        <div
          className={`relative ${isLarge ? "h-48" : "h-32"} overflow-hidden`}
        >
          <OptimizedImage
          ...
          />
          <div
            className={`absolute top-3 right-3 px-3 py-1 rounded-full text-white text-xs font-medium ${getCategoriaClass(categoria)}`}
          >
        ...
        <div className={`p-${isLarge ? "4" : "3"}`}>
          <h3
            className={`font-semibold text-texto-principal group-hover:text-categoria-default transition-colors line-clamp-${isLarge ? "3" : "2"} ${isLarge ? "text-lg" : "text-sm"}`}
          >
            {getPostTitle(post)}
          </h3>
        </div>
